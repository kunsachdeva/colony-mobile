...
export default class App extends Component {
  constructor() {
    super()
    this.state = { step: 0, status: 'GOOD CHOICE!', page: 0 }
  }

  render() {
    var { height, width } = Dimensions.get('window');
    return <View>
      {(this.state.page == 0) ? this.renderPage0() : this.renderPage1()}
    </View>
  }
  ...
  renderPage0() {
    ...
    return <View style={{ height, width, backgroundColor: '#ccdac9', alignItems: 'center', padding: 50 }}>
      ...
      {(this.state.step == 2) ? <Animatable.View animation="fadeInUp" direction="alternate" style={{ justifyContent: 'center', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 14, fontWeight: 'bold' }}>Paste is somewhere SAFE</Text>
